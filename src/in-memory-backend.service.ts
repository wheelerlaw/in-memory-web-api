...
import {
    BaseResponseOptions, BrowserXhr, Connection, ConnectionBackend,
...
export class InMemoryBackendService {
     ...
     */
    protected applyQuery(collection: {id: string | number}[], query: URLSearchParams): any[] {
    ...
    protected findById(collection: {id: string | number}[], id: number | string) {
        return collection.find((item: any) => item.id == id);
    }

    protected genId(collection: {id: string | number}[]): any {
        // If the Ids are alphanumeric strings, just generate a guid
        if (_.every(collection, (value, idx, coll) => isNaN(+value.id))) {
            return UUID.UUID()
        }

        // If the Ids in the list are numeric (canonical or strings), generate something numeric.
        // +item.id because I want this to work for both string and number types.
        let maxId: any = 0;
        collection.reduce((prev: any, item: any) => maxId = Math.max(maxId, +item.id), undefined);
        maxId++;

        // Determine if the Ids in the collection are canonical numbers or string numbers, return appropriate type
        maxId = (_.every(collection, (value, idx, coll) => typeof value.id === 'number')) ? maxId : maxId + '';
    ...
    protected indexOf(collection: {id: string | number}[], id: string | number) {
    ...
    protected parseId(collection: {id: string | number}[], id: string): any {
        // tslint:disable-next-line:triple-equals
    ...
    protected removeById(collection: {id: string | number}[], id: number | string) {
